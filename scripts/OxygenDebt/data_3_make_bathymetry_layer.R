...
bathy <- rename(bathy, depth = dybde)
bathy <- bathy[c("x", "y", "depth")]

# make into spatial points dataframe (note implicit utm34 in BALTIC_BATHY_BALTSEM.csv)
coordinates(bathy) <- c("x", "y")
proj4string(bathy) <- sp::CRS("+proj=utm +zone=34 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0")

# trim to extent of assessment units
helcom <- readOGR("data/OxygenDebt/shapefiles", "helcom_areas", verbose = FALSE)
bathy <- bathy[gIntersects(bathy, gUnaryUnion(helcom), byid = TRUE)[1,], ]

# join points with new helcom polygons
bathy$Basin <- over(bathy, helcom)$Basin

# check
if (FALSE) {
  plot(bathy, col = gplots::rich.colors(50, alpha=0.5)[cut(bathy$depth, 50)], pch = ".")
  plot(helcom, border = "red", add = TRUE)
}

# write
writeOGR(bathy[c("depth", "Basin")], "data/OxygenDebt/shapefiles", "helcom_bathymetry", driver = "ESRI Shapefile", overwrite_layer = TRUE)
